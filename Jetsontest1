...
# Devices on your Jetson
EO_DEV = "/dev/video0"
IR_DEV = "/dev/video2"

# Force IR to 1280x720 MJPEG@30 if available, then decode.
# If your IR cam does not support MJPEG@720p, replace the IR branch
# with a plain v4l2src ! videoconvert ! videoscale path (like EO).
pipeline_desc = f"""
...
eo_zoom = 2.0
EO_ZOOM_MIN = 2.0
EO_ZOOM_MAX = 21.0   # shows up to "Zoom 20.0x" (we display eo_zoom - 1)
...
def derive_ir(eo_val):
    ir_val = eo_val - 1.0
    if ir_val < IR_ZOOM_MIN:
        ir_val = IR_ZOOM_MIN
    if ir_val > IR_ZOOM_MAX:
        ir_val = IR_ZOOM_MAX
    return ir_val
...
def eo_step_down_clean(val):
    # mirror of up bands with snapping at edges
    if val > 10.0:
        new_val = val - 2.5
        if new_val < 10.0:
            new_val = 10.0
...
def reset_pads_and_crops():
    pad_cam_full.set_property("width", -1)
    pad_cam_full.set_property("height", -1)
    pad_ir_full.set_property("width", -1)
    pad_ir_full.set_property("height", -1)
    pad_cam_small.set_property("width", -1)
    pad_cam_small.set_property("height", -1)
    pad_ir_small.set_property("width", -1)
    pad_ir_small.set_property("height", -1)
...
# ----- Linux keyboard handling (SPACE / UP / DOWN) -----
...
class KB:
    ...
    def read_key(self):
        # non-blocking read; return b"UP", b"DOWN", b"SPACE", or None
...
print("SPACE: next layout | UP: zoom in | DOWN: zoom out | Ctrl+C to quit")
